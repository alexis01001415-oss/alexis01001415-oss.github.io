...
try {
  for (const [width, height] of [
    ...
    for (const phase of height < 600 ? [0] : [0, 1, 2]) {
      ...
      if (phase === 0 && [1440, 390, 768, 360].includes(width))
        await p.screenshot({ path: `artifacts/v3-hero-${width}.png` });
    }
    for (const selector of [
      ...
      if ([1440, 390].includes(width))
        await p.screenshot({
          path: `artifacts/v3-${selector.replace(/[^a-z]/g, '')}-${width}.png`,
        });
    }
    await p.evaluate(() =>
      scrollTo({
  ...
  await fs.writeFile('artifacts/qa-v3.json', JSON.stringify(report, null, 2));
  await b.close();
}
if (
